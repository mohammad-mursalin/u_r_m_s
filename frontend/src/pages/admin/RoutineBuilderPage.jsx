      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-4">
            ...
            >
              <RefreshCw size={16} className={checkingAllConflicts ? 'animate-spin' : ''} />
              Check All Conflicts
            </button>
            {selectedSemester && selectedSemester.is_active && (
              <button
            ...
            {selectedSemester && !selectedSemester.is_active && (
              <button
                onClick={() => handleUnpublish(selectedSemester.id)}
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <select
            ...
            onChange={(e) => handleSemesterChange(semesters.find(s => s.id === parseInt(e.target.value)))}
            className="border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            {semesters.map(semester => (
              <option key={semester.id} value={semester.id}>
                {semester.name}
                {semester.is_active && semester.is_published && ' (Active & Published)'}
                {semester.is_active && !semester.is_published && ' (Active Draft)'}
